                  ...
                  float chi2_threshold, float* chi2)
{
    ...
    matsymmetrize(P, n);  // Force symmetry: P = 0.5*(P + P')
...
void matsymmetrize(float* A, int n)
{
    // Force matrix symmetry by averaging corresponding elements
    // A = 0.5 * (A + A^T)
    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++) // Only upper triangular part
        {
            float avg = 0.5f * (MAT_ELEM(A, i, j, n, n) + MAT_ELEM(A, j, i, n, n));
            MAT_ELEM(A, i, j, n, n) = avg;
            MAT_ELEM(A, j, i, n, n) = avg;
        }
    }
}
